define(["jquery"], function ($) {
  ...
  $(".section__4 .tab__bot>a").click(function () {
    ...
    $(".section__4 .tab__content>div:nth-child(" + num + ")").addClass(
      "active"
    );
  });
  ...
  // TAB FUNCTIONAL START
  $(".tab-btn").on("click", function (e) {
    const target = $(this).attr("data-target");
    $(this).parent().children(".tab-btn").removeClass("select");
    $(this).toggleClass("select");

    $(target).parent().children(".tab-content").removeClass("show");
    $(target).toggleClass("show");
  });
  // TAB FUNCTIONAL END

  $(document).mouseup(function (e) {
    // web document click event
    var div = $(".help__menu"); // here we specify the ID of the element
    if (
      !div.is(e.target) && // if the click was not on our block
      div.has(e.target).length === 0
    ) {
      // and not by its children
  ...
  $(document).ready(function () {
    $("body").each(function () {
      ...
      img.onload = function () {
        var ppbox =
          '<div id="images__back" style="background: url(' +
          img_url +
          ") no-repeat 50% 0%;top:0 !important;width:100%;position:absolute;z-index:1000;opacity:.9;height:" +
          img.height +
          'px"></div>';
        body.append(ppbox);
      };
